...
export default function CounterPage (): JSX.Element {
  ...
  const log = (s: string) => setLogs(l => [s, ...l].slice(0, 40))
  ...
  useEffect(() => {
    void (async () => {
      ...
      try {
        ...
        log('init read failed: ' + String((e as any)?.message ?? e))
      }
    })()
  }, [dep])
  ...
  const buildAndSendUserOp = async (callData: string, label: string) => {
    if (!dep) return log('missing deployments')
    if (!owner) return log('paste owner private key')
    ...
    const nonceFacet = new ethers.Contract(dep.diamond, NONCE_ABI, provider)
    ...
    log('sending userOp: ' + label)

    await (await entryPoint.handleOps([op], beneficiary, { gasLimit: 8_000_000 })).wait()
    log('handleOps success: ' + label)
  }

  const sendUserOpCounterIncrement = async () => {
    if (!dep) return log('missing deployments')
    setBusy(true)
    try {
      ...
      await buildAndSendUserOp(callData, 'counter.increment')
      await readCounter()
      await refreshBalance()
    } catch (e: any) {
      log('error: ' + String(e?.message ?? e))
  ...
  const sendUserOpTransferEth = async () => {
    if (!dep) return log('missing deployments')
    if (!owner) return log('paste owner private key')
    setBusy(true)
    try {
      if (!ethers.utils.isAddress(toAddr)) throw new Error('bad recipient address')
      const value = ethers.utils.parseEther(amountEth || '0')
      const callData = new ethers.utils.Interface(EXEC_ABI).encodeFunctionData('execute', [toAddr, value, '0x'])
      await buildAndSendUserOp(callData, 'transfer ETH')
      await refreshBalance()
    } catch (e: any) {
      log('error: ' + String(e?.message ?? e))
    } finally {
  ...
  return (
    <div style={{ maxWidth: 900, margin: '40px auto', fontFamily: 'system-ui' }}>
      ...
      <button disabled={busy} style={{ marginTop: 12 }} onClick={() => { void sendUserOpCounterIncrement() }}>
        Send UserOp Counter increment
      </button>

      <hr />

      <h3>Transfer ETH via AA execute</h3>
      <input
      ...
      />
      <button disabled={busy} style={{ marginTop: 12 }} onClick={() => { void sendUserOpTransferEth() }}>
        Send UserOp Transfer ETH
      </button>
