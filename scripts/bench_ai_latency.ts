...
async function benchAILatency(runs: number, entryPoint: string, diamond: string, apiBase: string) {
  ...
  for (let i = 0; i < runs; i++) {
    ...
    const riskLatency = t1 - t0
    const chainLatency = t2 - t1
    const totalLatency = t2 - t0

    riskResults.push(riskLatency)
    chainResults.push(chainLatency)
    totalResults.push(totalLatency)

    console.log(
      `AI run ${i + 1}: risk=${riskLatency.toFixed(2)} ms, chain=${chainLatency.toFixed(2)} ms, total=${totalLatency.toFixed(2)} ms`
    )
  }

  return { riskResults, chainResults, totalResults }
}
...
async function main() {
  ...
  const { riskResults, chainResults, totalResults } = await benchAILatency(
    runs,
    entryPoint,
    diamond,
    apiBase
  ...
  summarize('AI Risk API', riskResults)
  summarize('AI Chain Execution', chainResults)
  summarize('AI Wallet Total', totalResults)
}
